...
// Remove otpStore and resend
code = code.replace(/const otpStore = new Map\(\);\n?/g, '');
code = code.replace(/import { Resend } from 'resend';\nconst resend = process\.env\.RESEND_API_KEY \? new Resend\(process\.env\.RESEND_API_KEY\) : null;\n?/g, '');

const startIdx = code.indexOf('app.post("/api/send-otp"');
const endIdx = code.indexOf('async function startServer()');
...
code = code.substring(0, startIdx) + newRoutes + code.substring(endIdx);
